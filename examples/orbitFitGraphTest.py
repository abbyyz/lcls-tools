...
#     ax[0].stem(o1['s'], np.array(o1[type1]['x']) * 1000, linefmt='#FF6F61', markerfmt='o', basefmt=" ", label=ty>
#     ax[0].plot(o2['s'], np.array(o2[type1]['x']) * 1000)
#     ax[0].plot(o2['s'][indx[0]:indx[-1]], np.array(o2[type1]['x'])[indx[0]:indx[-1]] * 1000, color='#007B7F', la>
#     ax[0].set_ylabel('x orbit [mm]')
#     ax[0].set_xlabel('s [m]')
#     ax[0].legend()
#     ax[1].stem(o1['s'], np.array(o1[type1]['y']) * 1000, linefmt='#FF6F61', markerfmt='o', basefmt=" ", label=ty>
#     ax[1].plot(o2['s'], np.array(o2[type1]['y']) * 1000)
#     ax[1].plot(o2['s'][indx[0]:indx[-1]], np.array(o2[type1]['y'])[indx[0]:indx[-1]] * 1000, color='#007B7F', la>
...
def on_click(event, o1, type1, ax0, ax1):
    ...
    if event.inaxes:  # Check if click is in any of the axes
        ...
        if closest_index < len(o1['element']):
            ...
            # Determine the correct axes to annotate
            if event.inaxes == ax0:  # Clicked on x orbit
                # Remove the previous annotation
                if current_annotation:
                    current_annotation.remove()
                    current_annotation = None  # Reset annotation reference

                # Place new annotation
                current_annotation = event.inaxes.annotate(element_info,
